...
import { lerp, linspace, map, join, clamp2D, clamp } from "./utils.js";

export class Aperture {
  #rect = $state(new AABB2(-1, -1, 1, 1));
  #fit = $state("meet");
  #alignX = $state("Mid");
  #alignY = $state("Mid");
...
export class Viewport {
  #camera = $state(new Camera());
  #screen = $state(new Screen());
  #scaled = $state(new AABB2());
  ...
  get scaledViewBox() {
    ...
    if (this.#camera.aperture.fit === "none") {
      ...
      const extraWeightingX = Aperture.alignmentWeight(
        this.#camera.aperture.alignY,
      );
      const extraWeightingY = Aperture.alignmentWeight(
        this.#camera.aperture.alignY,
      );
...
export class ViewportSVGAdapter {
  static #aspectRegExp = new RegExp(
    "^x(?<x>M(?:in|id|ax))Y(?<y>M(?:in|id|ax))(?:\\s+(?<fit>meet|slice|none))$",
  );
  static #space = /\s+/;
  ...
  set viewBox(string) {
    ...
    const [minX, minY, width, height] = string
      .split(ViewportSVGAdapter.#space, 4)
      .map(parseFloat);
  ...
  delegate(fn, ...args) {
    return (evt) =>
      fn(
        this.eventToWorld(evt, evt.currentTarget.ownerSVGElement),
        evt,
        this,
        ...args,
      );
  ...
  get visibleMin() {
    return this.screenToWorld(0, 0, 0, 0, this.width, this.height);
  }
  ...
  *linspaceX(padding, res = 1) {
    yield* linspace(
      this.visibleMin.x + padding,
      this.visibleMax.x - padding,
      this.visibleWidth * res,
    );
  }

  *linspaceY(padding, res = 1) {
    yield* linspace(
      this.visibleMin.y + padding,
      this.visibleMax.y - padding,
      this.visibleHeight * res,
    );
  }
}
